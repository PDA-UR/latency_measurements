...
ax = sns.stripplot(values, hue=None, jitter=True, dodge=False, color=None, palette="Set2", size=20, marker="D", alpha=0.25)
axes = plt.gca()
#axes.set_xlim([0,60])
#plt.gcf().subplots_adjust(bottom=0.15)
...
if len(sys.argv[1:]) == 1: # only one file
    ...
    plt.savefig(cur_dir + "/scatterplot_1ms/" + png_filename)
else:
    plt.show()
